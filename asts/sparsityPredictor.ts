import { OuroTopologyMap, OuroTensorRecord } from './topologyParser.js';

export interface GeometricRoutingPath {
    activatedLayers: number[];
    requiredTensors: OuroTensorRecord[];
    targetRank: number;
    dynamicCompressionRatio: number;
}

export class SparsityPredictor {
    private topology: OuroTopologyMap;

    constructor(topology: OuroTopologyMap) {
     ...
     */
    public predictRoutingPath(prompt: string): GeometricRoutingPath {
        ...
        const activatedLayers: number[] = [];
        const requiredTensors: OuroTensorRecord[] = [];
